...
@Component({
  selector: 'app-movimientos-cuentas', 
  templateUrl:'./movimientos.component.html',
  styleUrls: ['./movimientos.component.css'] 
})
export class movimientosCuentasComponent { 
  ...
  constructor( private cntService:CntContablesService , private serviceCaja:cajasServices){
    ...
    this.cntService.getCntCuentasConMovimientos().subscribe({next:(value:cntSubCuentaRequest)=>{
      if(value.numdata>0){
  ...
  filtrarCuentas(event: Event) {
    const inputElement = event.target as HTMLInputElement;
    const text = inputElement.value;
    let datos = [...this.cuentas];
    if(text != ''){
      datos =  datos.filter(x=>x.nro_scuenta?.toString().startsWith(text))
    }
    this.cuenta = [...this.cuentaIni ,  ...datos];
  }
  show(item:any){
 ...
 let arrayIdsCuenta:number[] = [this.cuentaSeleccionada];
    ...
    if(this.cuentaSeleccionada == 99999999){
      arrayIdsCuenta  = [];
      arrayIdsCuenta = this.cuenta.map(x=> x.id_scuenta!)
    }
