...
switch ($action){ 
    ...
    TRY{ 
       ...
       $link = $conexion->getLink(); 
        ...
        $parametros = array("fecha1"=>$_FECHA_1 , 
                            "fecha2" =>  $_FECHA_2, 
        ...
        $_result =$conexion->procedimiento('sp_generar_pago_empleado', $parametros   ); 
         $datos['$_result'] = $_result;
         if ($_result['_result'] === 'ok'){
             ...
             if(intval($datos['data'][0]['_result']) !== 100 ){
                 $datos['error']='error : error procedimiento getProductoConExistencia ' . $datos['datos'][0]['msg'];
                 echo json_encode($datos);
                 die();  
             } 
         }else{
         ...
         TRY{
           ...
           if ( !isset($_documento) && trim($_documento) == "" && !is_numeric($_documento)){
            ...
            $query = "update documentos set ".
                     ...
                     " where orden = {$_rst_documento['datos'][0]['orden']}";
    ...
    TRY{ 
       ...
       $link = $conexion->getLink(); 
        ...
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         ...
         $datos['$_producto_enviado'] = $_producto_enviado;
        $parametros = array("_id_producto"=>$_producto_enviado['id'] , 
                            "_id_caja" =>  $_documento['caja']
                     );
        $datos['$parametros'] = $parametros;
        $_result =$conexion->procedimiento('getProductoConExistencia', $parametros   ); 
         if ($_result['_result'] === 'ok'){
             $datos['data']= $_result['datos'];
             ...
             if(intval($datos['data'][0]['_result']) !== 100 ){
                 ...
                 echo json_encode($datos);
                 die();  
             }else{//3008940705 jaime
                  if($datos['data'][0]['cantidad'] >= $_producto_enviado['cantidadVendida'] ) {
               $queryFinal ="INSERT INTO `documentos_listado_productos`( ". 
                "`orden`,`idProducto`,`nombreProducto`,`presioVenta`,`porcent_iva`,`presioSinIVa`,".
                "`IVA`,`cantidadVendida`, `usuario`,`hora`,`cant_real_descontada`  ) ".
                       "VALUES ".
                "( {$_documento['orden']} , ".
                 "{$_producto_enviado['id']} , ".
                 "'{$_producto_enviado['nombre']}' , ".
                "{$_producto_enviado['precioVenta']} , ".
                "{$_producto_enviado['porcent_iva']} , ".
                "{$_producto_enviado['PrecioSinIVA']} , ".
                "{$_producto_enviado['valorIVA']} , ".
                "{$_producto_enviado['cantidadVendida']} , ".
                "{$_documento['usuario']} , ".
                "  now() , ".
                "{$_producto_enviado['cantidadVendida']}    ) " ;     
                    ...
                    if ($consulta->execute()){
                      ...
                      $datos['error']='error : La cantidad es superior a la existencia en bodega!!' ;
                 echo json_encode($datos);
                 die();  }  
    ...
    case 'devolver_producto_venta':
    TRY{ 
       $conexion =\Class_php\DataBase::getInstance();
       $link = $conexion->getLink();
     
         $parametros = array("_id_linea_devolucion"=>$_producto_enviado['id']);
        $datos['$parametros'] = $parametros;
        $_result =$conexion->procedimiento('sp_devolver_producto', $parametros   ); 
         if ($_result['_result'] === 'ok'){
             $datos['data']= $_result['datos'];
             ...
             //print_r($datos['datos'][0]);
             if(intval($datos['data'][0]['_result']) !== 100 ){
                 $datos['error']='error : error procedimiento sp_devolver_producto ' . $datos['datos'][0]['msg'];
                 echo json_encode($datos);
                 die();  
             }
    ...
    TRY {
                   ...
                   if (sizeof($_doc_obj->listado ) >0 ){
                     ...
                     foreach ($_doc_obj->listado as  $value) {
                         ...
                         if ($value->estado_linea_venta == 'A'){ 
                            $parametros = array("_id_linea_devolucion"=>$value->id);
                           $datos['$parametros'] = $parametros;
                           $_result =$conexion->procedimiento('sp_devolver_producto', $parametros   ); 
                            if ($_result['_result'] === 'ok'){
                                $datos['data']= $_result['datos'];
        ...
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         ...
         }else{  $datos['error']= 'Error al ejecutar procedimiento cancelarDocumento(). ' ;
           echo json_encode($datos);
           die();  }
    ...
    case 'DATABASE_GENERIC_CONTRUCT_PROCEDURE':
       ...
       $link = $conexion->getLink(); 
        ...
        break;
    case 'BORRAR_DATOS_INGRESO_AUX_INVENTARIO':
         TRY {
       $conexion =\Class_php\DataBase::getInstance();
       $link = $conexion->getLink(); 
         $_key = LLAVE_SESSION;
         ...
         $_result =$conexion->where('session', $where);
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         if (sizeof($_result['datos']) > 0 && isset($_bodega_ingreso) && $_bodega_ingreso >0 ){ 
             $_qazxswe = $_result['datos'][0]['usuario'];  
            $queryFinal = "DELETE FROM aux_ingreso_inventario WHERE "
                    . " bodega = {$_bodega_ingreso}"
                    . " and cod_usuario = {$_qazxswe} ";
            $consulta = $link->prepare($queryFinal);
            $datos['queryDelete'] = $queryFinal;
            $datos['error']='ok';
            //*aux_ingreso_inventario

            if (!$consulta->execute() ){
                $datos['error']= 'Error al intertar ingresar el nuevo producto ' ;
                echo json_encode($datos);
                die(); 
                } 
        
           }else{
            $datos['error']= 'Error de datos, no se encuentra el usuario logeado ' ;
           echo json_encode($datos);
           die();  
         }
     ...
     break;
      case 'INGRESO_DATOS_DATOS_AUX_INVENTARIO':
         TRY {
       $conexion =\Class_php\DataBase::getInstance();
       $link = $conexion->getLink(); 
         $_key = LLAVE_SESSION;
        ...
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         if (sizeof($_result['datos']) > 0  ){ 
             $_qazxswe = $_result['datos'][0]['usuario'];  
            $queryFinal = "INSERT INTO aux_ingreso_inventario ("
                    . "cod_producto, cod_usuario, cantidad, bodega ) "
                    . "VALUES ("
                    . "{$_ingreso['cod_producto']} , {$_qazxswe} , {$_ingreso['cantidad']} , {$_ingreso['bodega']} "
                    . ") ;";
            $consulta = $link->prepare($queryFinal);
            $datos['queryDelete'] = $queryFinal;
            $datos['error']='ok';
            ...
            if (!$consulta->execute() ){
                ...
                die(); 
                   ...
                   $where = " where cod_usuario = {$_qazxswe} and bodega = {$_ingreso['bodega']} " ;  
                  $_result =$conexion->where('vw_aux_ingreso_inventario', $where); 
                   if (sizeof($_result['datos']) > 0  ){ 
                          $datos['datos']= $_result['datos'];
                          $datos['data']= $_result['datos'];
                          $datos['numdata']= sizeof($_result['datos']);
      ...
      case 'INSERTAR_NUEVO_PRODUCTO':
       ...
       $link = $conexion->getLink(); 
        ...
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         if (sizeof($_result['datos']) > 0  ){ 
             $_qazxswe = $_result['datos'][0]['usuario'];  
            $_producto_enviado['barcode']  =  trim($_producto_enviado['barcode'] ) ;
            if(trim($_producto_enviado['barcode'] ) !== '' ) {
               $where = "  where  barcode = '".trim($_producto_enviado['barcode'] )."'   ";
               $_rst_asig =$conexion->where('prd_producto', $where  );  
               
              if (  ( sizeof($_rst_asig['datos']) > 0 
                      &&   ($_producto_enviado['id']<= 0)                         
                      ) or 
                      ( sizeof($_rst_asig['datos']) > 1
                      &&   ($_producto_enviado['id'] >  0) 
                      ) 
                 ...
                 $datos['error']= 'Error el codigo de barra a ingresar ya se encuentra registrado!!!' ;
                 echo json_encode($datos);
                 die(); 
              ...
              if($_producto_enviado['id']<= 0){
          $queryFinal = "insert into  `prd_producto` ".
                  "( usuario , idCategoria,idMarca, nombre, nombre2, nombre3, tipo_producto, "
                  . " precioVenta,  porcent_iva,  precioCompra,  barcode,  descripcion ) values ("
                  . "$_qazxswe , "
                  . "'{$_producto_enviado['idCategoria']}', '{$_producto_enviado['idMarca']}', "
                  . "'{$_producto_enviado['nombre']}', '{$_producto_enviado['nombre2']}', "
                  . "'{$_producto_enviado['nombre3']}', '{$_producto_enviado['tipo_producto']}', "
                  . "'{$_producto_enviado['precioVenta']}', '{$_producto_enviado['porcent_iva']}', "
                  . "'{$_producto_enviado['precioCompra']}', '{$_producto_enviado['barcode']}', "
                  . "'{$_producto_enviado['descripcion']}' "
              . ")";
                  ...
                  /*  "( usuario , idCategoria,idMarca, nombre, nombre2, nombre3, tipo_producto, "
                  . " precioVenta,  porcent_iva,  precioCompra,  barcode,  descripcion ) values ("*/    
                  $queryFinal = "update `prd_producto` set "                
                  . "usuario = $_qazxswe , "
                  . "idCategoria = '{$_producto_enviado['idCategoria']}', idMarca=  '{$_producto_enviado['idMarca']}', "
                  . "nombre ='{$_producto_enviado['nombre']}', nombre2 = '{$_producto_enviado['nombre2']}', "
                  . " nombre3= '{$_producto_enviado['nombre3']}', tipo_producto = '{$_producto_enviado['tipo_producto']}', "
                  . "precioVenta = '{$_producto_enviado['precioVenta']}', porcent_iva = '{$_producto_enviado['porcent_iva']}', "
                  . "precioCompra = '{$_producto_enviado['precioCompra']}', barcode = '{$_producto_enviado['barcode']}', "
                  . "descripcion = '{$_producto_enviado['descripcion']}' where id =  {$_producto_enviado['id']}" ;
              }
                    $consulta = $link->prepare($queryFinal);
                    $datos['queryDelete'] = $queryFinal;
                     $datos['error']='ok';
    ...
    case 'CREAR_DOCUMENTO_POR_SERVICIO_VEHICULO':
           TRY {
        $flagDocumento = false ; 
       $conexion =\Class_php\DataBase::getInstance();
       $link = $conexion->getLink(); 
         ...
         if (sizeof($_result['datos']) > 0){ 
             $_qazxswe = $_result['datos'][0]['usuario'];  
             $datos['usuario'] = $_qazxswe;
             if(isset($_arraydatos['idDocumento']) && intval($_arraydatos['idDocumento'])> 0){
                 $_result['_result'] = 'ok';
                 $datos['datos'][0]['_result'] = 100;
                 $datos['datos'][0]['idIngresado'] = $_arraydatos['idDocumento'];
                 $flagDocumento = true;
             }else{
             
              $_result =$conexion->procedimiento('crearNuevoDocumentoPorIngresoVehiculo',
                      array("usuario" => $_qazxswe,
                  "cajaId" => $_arraydatos['cajaAsignada']));
             }       
         if ($_result['_result'] === 'ok'){
             if (!$flagDocumento){
             $datos['datos']= $_result['datos'];
             ...
             if(intval($datos['datos'][0]['_result']) !== 100 ){
                 $datos['error']=$datos['datos'][0]['msg'];
                  ...
                  $_arraydatos['idDocumento']=$datos['datos'][0]['idIngresado'];
                 $queryIngreso = 
                    'insert into vehiculos_servicios_ingresos  '.
                    '(cod_servicio, propietario, telefono, cod_tipo_vehiculo, lavador,    cajaAsignada, idDocumento, valor, '
                         . 'placaVehiculo , usuario ) values '.
                    "( {$_arraydatos['cod_servicio']} ".
                    ", '{$_arraydatos['propietario']}' ".
                    ", '{$_arraydatos['telefono']}' ".
                    ", {$_arraydatos['cod_tipo_vehiculo']} ".
                    ", {$_arraydatos['lavador']} ".
                    ", {$_arraydatos['cajaAsignada']} ".
                    ", {$_arraydatos['idDocumento']} ".
                    ", {$_arraydatos['valor']} ".
                    ", '{$_arraydatos['placaVehiculo']}' ,{$_qazxswe} ".
                    ")";   
                    $datos['idDocumento'] = $_arraydatos['idDocumento'] ; 
                     $consulta = $link->prepare($queryIngreso);
                    $datos['queryIngreso'] = $queryIngreso;
                    if ($consulta->execute()){
                      $datos['error']='ok'; 
                    }else{ $datos['error']="Error al tratar de ingresar los valores a la base de datos TABLA (vehiculos_servicios_ingresos)"; }
 ...
 TRY {
       ...
       $_key = LLAVE_SESSION;
        ...
        // $datos['11'] =  $_result['datos'][0]['usuario'];;
         ...
         if(isset($_deleteBefore) && is_array($_deleteBefore)){
             $datosWhere = ' WHERE ';
             $AND = '';
             foreach ($_deleteBefore as $key => $value) {
                   $valor[2] = "'$valor[2]'";   
                 $datosWhere .= " $AND `$value[0]` $value[1] $value[2] ";  
                 $AND = 'AND';
             }
                     
             $queryFinal = "delete from `$_tabla`   $datosWhere ";
                    $consulta = $link->prepare($queryFinal);
                    $datos['queryDelete'] = $queryFinal;
                   $consulta->execute() ;
         }
         $_parent_ini = '(';
        foreach ($_arraydatos as $clave => $dato){
            
             $valor= $dato;
             if ($valor[0] === 'USUARIO_LOGUEADO'){
               $valor[0] = $_qazxswe ;
             }
             if ($valor[1] === '')
             {$valor[0] = "'$valor[0]'";}
             ...
             $_valores  .= "  $_coma $valor[0]";
             $_parent_ini = '';
            $_coma = ',';
        }
         
$queryFinal = "INSERT INTO  `$_tabla` ( $_columnas ) select $_valores from `$_tablaSelect`; ";
                    $consulta = $link->prepare($queryFinal);
                        ...
                        foreach ($_pagos as $key => $value) {
                           ...
                           $queryFinal =  "insert into documentos_pagos "
                            ...
                            if ($consulta->execute()){
                                ...
                                . "la base de datos TABLA (documentos_pagos)";  echo json_encode($datos);
                                 die();}
                                
                        }}
                    

                    }else{ $datos['error']="Error al tratar de limpiar los valores a la base de datos TABLA (documentos_pagos)"; }
 }
      catch (PDOException $e) {
           http_response_code(500);
        $datos['error']= 'Error de conexión: ' . $e->getMessage();
    ...
    */
    ...
    case 'e06c06e7e4ef58bdb0kieujfñ541b3017fdd35473'://select .
        /*
     
    action = e06c06e7e4ef58bdb0kieujfñ541b3017fdd35473
    _tabla = array con los nombres de las tablas 
    ...
    */
       ...
       $limit = null;
       if (isset($_limit) and sizeof($_limit)> 0 and is_array($_limit)){
       ...
       if (isset($_where) and sizeof($_where)> 0 and is_array($_where)){
         ...
         foreach ($_where as $clave => $valor){
             ...
             if (trim($valor['tipocomp']) == '=f'){
                 $_dato = "{$valor['dato']}";
                 $valor['tipocomp'] = '=';
             }
            $where.= " $and {$valor['columna']} {$valor['tipocomp']} $_dato "; 
            if(isset($valor['relacion'])   ){
                switch ($valor['relacion']) {
                    case 'or':
                    case 'OR':
                        $and = ' OR ';
                        break;
      ...
      $datos[$key]['data']= $_result['datos'];
      $datos[$key]['query']= $_result['query'];
      
      $datos[$key]['numdata']= sizeof($_result['datos']);
      $datos[$key]['error'] ='ok';  
}
    ...
    */
      ...
      if  (isset($_tablau) && trim($_tablau) != ''){
          $_result =$conexion->where_union($_tabla, $where,$_tablau, $whereu , NULL , $limit);
      }else{
      $_result =$conexion->where($_tabla, $where,$columnas , $limit );}
      /*$array['datos'] =  $consulta->fetchAll();
         $array['query'] */
    ...
    */
       ...
       if (isset($_where) and sizeof($_where)> 0 and is_array($_where)){
         ...
         foreach ($_where as $clave => $valor){
            ...
            $where.= " $and {$valor['columna']} {$valor['tipocomp']} $_dato ";  
            if(isset($valor['relacion']) && trim($valor['relacion']) != ''){
                $valor['relacion'] = trim($valor['relacion']);
                $valor['relacion'] = strtoupper($valor['relacion']);
                
                $auxConj = $valor['conjuncion'];
                echo $auxConj;
                switch ($auxConj) { 
                     case 'OR' : 
                        $and = ' OR ';
